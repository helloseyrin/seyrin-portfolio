...
const acid = [
  { abbr: "A", term: "Atomicity",   def: "A transaction either completes in full or doesn't happen at all — no partial writes, no half-committed state left behind. I wrap multi-step operations in explicit transactions so that if anything fails mid-process, everything rolls back cleanly.", color: "#8aaac8" },
  { abbr: "C", term: "Consistency", def: "Every transaction must leave the database in a valid state, respecting every constraint, rule, and relationship defined in the schema. I enforce constraints at the database level rather than relying on application logic alone — the schema is the last line of defence.", color: "#8aaac8" },
  { abbr: "I", term: "Isolation",   def: "Concurrent transactions should behave as if they ran one after another, even when they're happening at the same time. When designing pipelines that touch shared data, I think carefully about isolation levels and where phantom reads or race conditions could silently corrupt results.", color: "#8aaac8" },
  { abbr: "D", term: "Durability",  def: "Once committed, a transaction stays committed — power loss, crash, restart, none of it undoes it. I don't treat a queued or in-flight write as confirmed, and I rely on write-ahead logging rather than assuming memory is enough.", color: "#8aaac8" },
];

const base = [
  { abbr: "BA", term: "Basically Available",  def: "The system stays responsive even if it can't guarantee perfectly fresh data — availability is the primary promise, not strict correctness. For read-heavy workloads where slightly stale results are acceptable, I favour this over locking everything down and risking unavailability.", color: "#a78bfa" },
  { abbr: "S",  term: "Soft state",           def: "Nodes in a distributed system are allowed to temporarily disagree — state drifts and is reconciled over time rather than synchronised at every moment. I design with the expectation that replicas may lag, and I don't build logic that assumes instant consistency across nodes.", color: "#a78bfa" },
  { abbr: "E",  term: "Eventually consistent", def: "All nodes will converge to the same value given enough time and no new writes — it just won't be instant. I use this model for caches and non-critical data, and I'm deliberate about where I don't use it: anything involving integrity or financial data gets stricter guarantees.", color: "#a78bfa" },
];
